...
PNG = base64.b64decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aMK0AAAAASUVORK5CYII=')
...
with tempfile.TemporaryDirectory(prefix='veyra-pages-') as temporary:
    ...
    try:
        with sync_playwright() as p:
            ...
            before = page.evaluate("document.getElementById('viewer-canvas').toDataURL()")
            page.evaluate("veyra.document().setParam('node-grade', 'exposure', 0.75)")
            page.wait_for_function("previous => document.getElementById('viewer-canvas').toDataURL() !== previous", arg=before, timeout=30000)
